...
parameter	P0_ADDR		= 32'hEC_8000,//14718880//1C12F4000,//32'h0004_6600,		//第一幅图片P0的首扇区地址
			P_MEM		= 32'h7_8000,//32'h0007_5800,		//一副800*600的8位图片格式在SD卡中所占用的地址空间
			LAST_ADDR	= 32'hF4_0000,//1C17A4000;//32'h004d_d600;		//10幅图片的最后一个地址
...
parameter	CMD_IDLE	= 4'd0,		//无命令发送，等待状态
			...
			CMD_CSH 	= 4'd13,	//T0时刻CS拉高，并送8个周期clk
			CMD_FE		= 4'd14,	//发送FE以示写开始
			CMD_WR      = 4'd15;    //写512Byte状态
...
always @(cmd_cstate or wait_cnt8 or cmd_clk or cmd_en or spi_tx_rdy or spi_rx_rdy or nclk_cnt or retry_rep
	...
	case(cmd_cstate)
			...
			CMD_WR: begin
				if(cnt512 == 10'd514) cmd_nstate <= CMD_DELAY;	//直到写入512字节+2字节CRC完成
